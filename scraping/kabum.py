...
def buscar_kabum(entrada):
    ...
    for item_kb in items_kb:
        ...
        try:
            rating = item_kb.find_element(
                By.CLASS_NAME,
                'text-xxs.text-black-600.leading-none.pt-4'
            ).text
            partes = rating.replace('Avaliações:', '').strip().split(' ')
            if len(partes) >= 2:
                rating = float(partes[0].replace(',', '.'))
                reviews = int(partes[1].replace('(', '').replace(')', '').replace('.', ''))
        except:
            pass
